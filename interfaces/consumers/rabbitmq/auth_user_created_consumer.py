...
from interfaces.consumers.rabbitmq.handlers.auth_user_created_handler import handle_auth_user_created_event

QUEUE_NAME = "auth.user_created.queue"
...
async def on_message(message: AbstractIncomingMessage):
    try:
        async with message.process(ignore_processed=True):
            ...
            print(payload)
            await handle_auth_user_created_event(payload)
...
async def handle_retry_logic(message: AbstractIncomingMessage, exc: Exception):
    ...
    if retry_count < MAX_RETRIES:
        ...
        retry_exchange = await channel.get_exchange("auth.user_created.direct")# type: ignore
...
async def run_auth_user_created_consumer():
    ...
    arguments: Arguments = {
        'x-dead-letter-exchange': 'auth.user_created.dlx'
    }
